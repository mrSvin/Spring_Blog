...
public interface UsersRepository extends CrudRepository<User, Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE `users` SET `email` = ?1, `name` = ?2, `password` = ?3, `photo` = ?4 WHERE (`id` = ?5)", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE `users` SET `email` = ?1, `name` = ?2, `password` = ?3 WHERE (`id` = ?4)", nativeQuery = true)
    ...
    @Query(value="SELECT email FROM users where email = ?1 and id != ?2", nativeQuery = true)
    ...
    public void newPassword(@Param("password") String password,  @Param("code") String code);
