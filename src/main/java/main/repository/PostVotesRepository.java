...
public interface PostVotesRepository extends CrudRepository<PostVotes, Integer> {
    ...
    @Query(value="SELECT COUNT(value) FROM post_votes  where post_id =?1 AND value =?2 AND user_id =?3", nativeQuery = true)
    public Integer findUserVotesLikes(int postId, int typeLike, int userId);

    @Modifying
    @Transactional
    @Query(value = "INSERT INTO `post_votes` (`time`, `value`, `post_id`, `user_id`) VALUES (?1, ?2, ?3, ?4)", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE `post_votes` SET `value` = ?1 WHERE (`post_id` = ?2 and `user_id` = ?3)", nativeQuery = true)
    public void changeLike(@Param("likeValue") Integer likeValue, @Param("postId") Integer postId, @Param("userId") Integer userId);

    @Query(value="SELECT count(*) FROM post_votes where value = 1 and user_id = ?1", nativeQuery = true)
